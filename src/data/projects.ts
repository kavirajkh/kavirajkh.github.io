...
export const projects: Project[] = [
  ...
  },
  {
    ...
    tradeoffs:
      "With a live $3B/year benefits program and 4 million active users, a big-bang cutover was not an option. The team shipped five applications iteratively across a two-year modernization program instead of one monolithic release, which meant running legacy and modernized components side by side for extended periods, adding integration complexity in exchange for the ability to validate each piece against real student and institution traffic before full cutover. That incremental approach also surfaced a production memory leak that only manifested under sustained load from one of the newly migrated applications; heap-dump analysis traced it to a caching layer that hadn't been sized for the new traffic pattern, and the fix shipped without affecting the wider rollout schedule.",
    impact: [
